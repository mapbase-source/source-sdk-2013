...
static char g_szDefaultNPC[128] = ""; // глобальна€ переменна€ дл€ хранени€ default_npc на клиенте
...
class CDialoguePanel : public vgui::Frame
{
	...
	// Animation phases:
	//   ANIM_NONE         Ч idle, no animation
	//   ANIM_SLIDE_IN     Ч panel slides up from off-screen to final position, fading in
	//   ANIM_BUTTONS_FADE Ч buttons gradually become visible after typewriter finishes
	//   ANIM_SLIDE_OUT    Ч panel slides down off-screen, fading out (on close)
	...
	: BaseClass(NULL, "DialoguePanel")
{
	...
	// Character name label Ч white text
	...
	// Separator line Ч drawn manually in PaintBackground for reliability
	m_pSeparator = new Panel(this, "DiagSeparator");
	m_pSeparator->SetVisible(false);

	// Dialogue rich text Ч background drawn manually in PaintBackground
...
void CDialoguePanel::ShowPanel(void)
{
	...
	// Hide buttons Ч they fade in after typewriter finishes
	for (int i = 0; i < 5; i++)
		m_pOptions[i]->SetAlpha(0);

	// Pause typewriter during slide-in Ч it will resume when slide-in finishes.
...
void CDialoguePanel::HidePanel(void)
{
	...
	// Cancel any pending deferred hide Ч we're handling it now
	m_bHidePending = false;
...
void CDialoguePanel::SkipTypewriter(void)
{
	...
	// Text finished Ч fade buttons in
	BeginButtonsFade();
}
...
void CDialoguePanel::LookAtTarget(const char* targetName)
{
	...
	// Always ask server Ч handles NPC look-at-player, and provides position for
...
void CDialoguePanel::PerformLayout()
{
	...
	// Layout: dialog box on top, gap, then buttons below Ч all inside one VGUI panel.
	...
	// Only update if changed Ч prevents infinite invalidation loop
...
void CDialoguePanel::PaintBackground()
{
	...
	// Black background Ч only for the dialog box area (not the buttons below)
...
void CDialoguePanel::OnTick()
{
	...
	if (m_eAnimPhase == ANIM_SLIDE_IN)
	{
		...
		if (flFraction >= 1.0f)
		{
			// Slide-in finished Ч snap to final position
			m_eAnimPhase = ANIM_NONE;
			SetAlpha(255);
			...
			{
				// No typewriter text Ч fade buttons in immediately
				BeginButtonsFade();
			}
		}
	...
	if (m_eAnimPhase == ANIM_BUTTONS_FADE)
	{
		...
		// Don't return Ч fall through so camera tracking continues during button fade
	}
	...
	if (m_eAnimPhase == ANIM_SLIDE_OUT)
	{
		float flElapsed = gpGlobals->realtime - m_flAnimStartTime;
		float flFraction = clamp(flElapsed / DIALOGUE_ANIM_DURATION, 0.0f, 1.0f);
		// Ease-in curve (accelerating) Ч reverse of slide-in's ease-out
		float flSmooth = flFraction * flFraction;
	...
	if (m_bShouldTrackTarget && m_szFocusTargetName[0])
	{
		...
		if (pPlayer)
		{
			...
			{
				// Entity not on client (info_target, etc.) Ч request updated position from server
	...
	if (m_bTypewriterActive && m_szTypewriterBuffer[m_iTypewriterPos] != '\0')
	{
		...
		while (m_szTypewriterBuffer[m_iTypewriterPos] == '<')
		{
			...
			// Not a recognized tag Ч break out and print '<' as a character
			if (!bHandled)
				break;
		...
		if (flNow - m_flTypewriterLastCharTime >= flCharInterval)
		{
			...
			if (m_szTypewriterBuffer[m_iTypewriterPos] != '\0')
			{
				...
				// Play typewriter sound Ч exactly one sound per character
...
void CDialoguePanel::OnCommand(const char* pcCommand)
{
	...
	// Don't call BaseClass::OnCommand Ч we handle all commands ourselves.
...
void CDialoguePanel::ShowNode(const char* nodeName)
{
	...
	// Hide buttons for the new node Ч they'll fade in after text finishes
	for (int i = 0; i < 5; i++)
	{
