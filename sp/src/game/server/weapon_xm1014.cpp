...
//	float	GetFireRate(void) { return 0.08f; }	// 13.3hz
	...
	virtual const Vector& GetBulletSpread(void)
	{
		...
		{
			// Give Alyx's xm1014 blasts more a more directed punch. She needs
	...
	virtual float	GetFireRate(void);// 13.3hz
...
{
	{ ACT_IDLE, ACT_IDLE_SMG1, true },	// FIXME: hook to xm1014 unique
...
//-----------------------------------------------------------------------------
// Purpose:	When we shipped HL2, the xm1014 weapon did not override the
...
//			we are going to fix the combine's rate of xm1014 fire in episodic.
//			This change will not affect Alyx using a xm1014 in EP1. (sjb)
//-----------------------------------------------------------------------------
...
bool CWeaponXm1014::StartReload(void)
{
	...
	// If xm1014 totally emptied then a pump animation is needed
...
//-----------------------------------------------------------------------------
// Purpose: Override so xm1014 can do mulitple reloads in a row
//-----------------------------------------------------------------------------
void CWeaponXm1014::ItemPostFrame(void)
{
	...
	// Xm1014 uses same timing and ammo for secondary attack
