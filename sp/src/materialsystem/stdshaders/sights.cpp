...
BEGIN_SHADER_PARAMS
SHADER_PARAM(PROJECTION_DEPTH, SHADER_PARAM_TYPE_FLOAT, "", "")
...
SHADER_INIT_PARAMS()
{
	if(!params[PROJECTION_DEPTH]->IsDefined())
		params[PROJECTION_DEPTH]->SetFloatValue(1.0f);
...
//		int pTexCoordDim[3] = { 2, 0, 3 };
		...
		pShaderShadow->VertexShaderVertexFormat(nFlags, nTexCoords, 0, 0);
...
//		BOOL BBools[16] = { false };
		...
		f4EyePos[3] = params[PROJECTION_DEPTH]->GetFloatValue();

		// c0
		pShaderAPI->SetPixelShaderConstant(0, f4EyePos);

		float f4DetailTint[4];
		params[DETAILTINT]->GetVecValue(f4DetailTint, 3);
		pShaderAPI->SetPixelShaderConstant(1, f4DetailTint);

		float f4ColorTint[4];
		ComputeModulationColor(f4ColorTint);
		pShaderAPI->SetPixelShaderConstant(2, f4ColorTint);
